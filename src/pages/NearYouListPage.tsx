...
const NEAR_YOU_EVENTS: NearYouEvent[] = [
  ...
  },
  {
    ...
    categories: ['Shows and culture', 'Concerts and festivals'],
    eventTag: 'Fever Original',
    paymentType: 'redeem',
    points: '1.000.000',
    hasTimer: true,
    msLeft: 50 * 24 * 60 * 60 * 1000,
    route: '#redeem',
  },
  {
    id: 'ny3',
    title: 'Linkin Park World Tour at Stade de France',
    date: 'March 7, 2026',
  ...
  {
    id: 'ny4',
    title: 'Spa & Wellness Day at Sofitel Paris Le Faubourg',
    date: 'March 7, 2026',
  ...
  {
    id: 'ny8',
    title: 'Monaco Grand Prix Fairmont Hairpin Experience',
    date: 'March 7, 2026',
  ...
  {
    id: 'ny10',
    title: 'Tour de France VIP Finish Line Experience',
    date: 'March 7, 2026',
  ...
  },
  {
    ...
    date: 'April 12, 2026',
    image: 'https://images.unsplash.com/photo-1513593771513-7b58b6c4af38?w=400&h=400&fit=crop',
    categories: ['Sport and leisure'],
  ...
  {
    id: 'ny12',
    title: 'Moulin Rouge Dinner & Show Experience',
    date: 'April 5, 2026',
...
function parsePrice(event: NearYouEvent): number {
  if (event.cashPrice) return parseFloat(event.cashPrice.replace(/[^\d,]/g, '').replace(',', '.'));
  if (event.points) return parseFloat(event.points.replace(/\./g, '').replace(',', '.'));
  return 0;
}
...
function LiveTimer({ initialMs }: { initialMs: number }) {
  ...
  return (
    <div className="ny-list__card-timer">
      <span className="ny-list__card-timer-label">Time left:</span>
...
export interface NearYouListPageProps {
  cityName?: string;
}
...
export default function NearYouListPage({ cityName = 'Paris' }: NearYouListPageProps) {
  ...
  const getChipLabel = (label: string): string => {
    if (label === 'Date' && selectedDate !== null) return `${selectedDate} ${MONTH_NAMES[calMonth].slice(0, 3)}`;
    ...
    if (label === 'Location' && selectedCity) return selectedCity;
  ...
  const filteredBrands = HOTEL_BRANDS.filter((b) => b.toLowerCase().includes(brandSearch.toLowerCase()));
  const nearbyCities = getNearbyCities(CURRENT_COUNTRY);
  const filteredCities = citySearch.trim() ? searchCities(citySearch) : nearbyCities;
            ...
            <div className="loyalty-sheet__body">
              <div className="loyalty-sheet__info">
                ...
                <div className="loyalty-sheet__info-col">
                  ...
                  <span className="loyalty-sheet__value">{userPoints.toLocaleString('de-DE')}</span>
      ...
      <div className="category-page__content">
        ...
        <nav className="category-page__breadcrumb" aria-label="Breadcrumb">
          <a
            ...
            onClick={(e) => { e.preventDefault(); navigateTo(`#city/${citySlug}`); }}
          >
            <IconChevronLeft />
            <span className="category-page__breadcrumb-back-label">{cityName}</span>
          </a>
          <span className="category-page__breadcrumb-trail">
            <a
              href={`#city/${citySlug}`}
          ...
          <span className="category-page__breadcrumb-current">Near you</span>
        </nav>

        {/* Title */}
        <div className="category-page__header">
          <h1 className="category-page__title">Near you</h1>
        </div>
        ...
        <div className="category-page__results-bar">
          ...
          <button type="button" className="category-page__order-btn" onClick={() => setOrderOpen(true)}>
            {sortBy === 'relevance' ? 'Sort by' : SORT_OPTIONS.find((o) => o.value === sortBy)!.label}
            <IconOrderBy />
          </button>
        </div>

        {/* Horizontal cards list */}
        <div className="ny-list__cards">
          {filteredEvents.map((event) => {
            const href = event.route ?? '#standard';
            const label = paymentLabel(event.paymentType);
            return (
              <article
                key={event.id}
                className="ny-list__card"
            ...
            <div className="filter-sheet__body">
                ...
                <div className="filter-check-list">
                  {CATEGORIES.map((cat) => (
                    <label key={cat} className="filter-check-list__item">
                      <input type="checkbox" className="filter-check-list__checkbox" checked={filterCategories.has(cat)} onChange={() => toggleSetItem(setFilterCategories, cat)} />
                ...
                <div className="filter-check-list">
                  {PAYMENT_OPTIONS.map((opt) => (
                    <label key={opt} className="filter-check-list__item">
                      <input type="checkbox" className="filter-check-list__checkbox" checked={filterPayments.has(opt)} onChange={() => toggleSetItem(setFilterPayments, opt)} />
              ...
              {activeFilter === 'hotel' && (
                <>
                  <div className="filter-search">
                    <IconSearch />
                    <input type="text" className="filter-search__input" placeholder="Search Hotel Brand" value={brandSearch} onChange={(e) => setBrandSearch(e.target.value)} />
                  </div>
                  <div className="filter-check-list">
                    {filteredBrands.map((brand) => (
                      <label key={brand} className="filter-check-list__item">
                        <input type="checkbox" className="filter-check-list__checkbox" checked={filterBrands.has(brand)} onChange={() => toggleSetItem(setFilterBrands, brand)} />
              ...
              {activeFilter === 'location' && (
                <>
                  <div className="filter-search">
                    <IconSearch />
                    <input type="text" className="filter-search__input" placeholder="Search for City" value={citySearch} onChange={(e) => setCitySearch(e.target.value)} />
                  </div>
                  <div className="filter-city-list">
                    <span className="filter-city-list__heading">{citySearch.trim() ? 'Results' : 'Nearby cities'}</span>
