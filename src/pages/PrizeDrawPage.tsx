...
const DEFAULT_DRAW_MS_LEFT = (21 * 24 * 60 + 5 * 60 + 34) * 60 * 1000;
...
const DEFAULT_TICKET_PRICE = 500;
...
export default function PrizeDrawPage({ eventId }: { eventId?: string }) {
  ...
  const DRAW_MS_LEFT = eventData?.msLeft ?? DEFAULT_DRAW_MS_LEFT;
  const USER_POINTS = userPoints;
  const TICKET_PRICE = eventData?.ticketPrice ?? DEFAULT_TICKET_PRICE;
  ...
  const isLessThan24h = useMemo(() => {
    return DRAW_MS_LEFT <= 24 * 60 * 60 * 1000;
  }, [DRAW_MS_LEFT]);

  const ticketSection = (variant: 'mobile' | 'sidebar') => (
    <section
      className={`prize-draw__ticket-section${variant === 'mobile' ? ' prize-draw__ticket-section--mobile' : ''}`}
    >
      {drawEnded && (
        <span className="auction-page__ended-badge">Ended</span>
      )}

      <div className="prize-draw__clock-wrap">
        <div className="prize-draw__time-box">
          <span className="prize-draw__time-label">Prize draw ends</span>
          <span className="prize-draw__time-date">
        ...
        {isLessThan24h && !drawEnded && (
          <div className="prize-draw__urgency-message" role="status">
            Less than 24 hours left. Buy your ticket now!
          </div>
        )}
      </div>
      ...
      {!drawEnded && (
        <>
          ...
          <div className="prize-draw__stepper">
            <button
              type="button"
              className={`prize-draw__stepper-btn${ticketCount <= 1 ? ' prize-draw__stepper-btn--disabled' : ''}`}
            ...
            <span className="prize-draw__stepper-value">
              {ticketCount} ticket{ticketCount > 1 ? 's' : ''}
            </span>
            <button
              type="button"
              className={`prize-draw__stepper-btn${ticketCount >= MAX_TICKETS ? ' prize-draw__stepper-btn--disabled' : ''}`}
      ...
      <div className="auction-page__content-layout">
        <main className="auction-page__main">
          ...
          <section className="auction-page__section">
            ...
            <p className="auction-page__body">
              Participation in the Prize Draw is open to individuals aged 18 years or older (or the legal age of majority in their jurisdiction). Employees of ACCOR, its affiliates, partners...
            </p>
